...
class DQN:
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            model = tf.layers.conv2d(self.input_X, 32, [4, 4],
                                     padding='same', activation=tf.nn.relu)
            model = tf.layers.conv2d(model, 64, [2, 2], padding='same', activation=tf.nn.relu)
            model = tf.contrib.layers.flatten(model) # 다차원을 1차원 텐서로 변경
            model = tf.layers.dense(model, 512, activation=tf.nn.relu)  # FC layer
